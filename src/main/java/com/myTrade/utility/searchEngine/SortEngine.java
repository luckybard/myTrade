...
//    }
    ...
    private static List<AdEntity> fromOldestSort(List<AdEntity> adEntityList) {
        return adEntityList.stream().sorted(Comparator.comparing(AdEntity::getIsHighlighted).thenComparing(AdEntity::getCreatedDateTime)).collect(Collectors.toList());
    }

    private static List<AdEntity> fromNewestSort(List<AdEntity> adEntityList) {
        return adEntityList.stream().sorted(Comparator.comparing(AdEntity::getIsHighlighted).thenComparing(AdEntity::getCreatedDateTime).reversed()).collect(Collectors.toList());
    }

    private static List<AdEntity> fromCheapestSort(List<AdEntity> adEntityList) {
        return adEntityList.stream().sorted(Comparator.comparing(AdEntity::getIsHighlighted).thenComparing(AdEntity::getPrice)).collect(Collectors.toList());
    }

    private static List<AdEntity> fromMostExpensiveSort(List<AdEntity> adEntityList) {
        return adEntityList.stream().sorted(Comparator.comparing(AdEntity::getIsHighlighted).thenComparing(AdEntity::getPrice).reversed()).collect(Collectors.toList());
    }
}
